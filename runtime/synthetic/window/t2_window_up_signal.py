...
UP_THRESHOLDS: dict[str, list[tuple[float, float]]] = {
    "ETH-USD": [
...
def best_up_sigma(pair: str, pct_diff: float | None) -> float | None:
    if pct_diff is None:
        return None
    min_sigma = MIN_SIGMA[pair]
    best: float | None = None
    for sigma, threshold in UP_THRESHOLDS[pair]:
        if sigma < min_sigma:
            continue
...
def main() -> int:
    ...
    sigmas = {pair: best_up_sigma(pair, pct_diffs.get(pair)) for pair in PAIRS}
