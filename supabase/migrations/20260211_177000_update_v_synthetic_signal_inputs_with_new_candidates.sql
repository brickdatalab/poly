...
select
  ...
  -- Newly appended columns (kept at end to avoid replacing existing view column order)
  max(v1) filter (where config_id = 'syn_window_edge_57to01_eth_nonrolling_tplus2') as syn_window_edge_57to01_eth_nonrolling,
  max(v2) filter (where config_id = 'syn_window_edge_57to01_eth_nonrolling_tplus2') as syn_window_edge_dir_code,
  max(v3) filter (where config_id = 'syn_window_edge_57to01_eth_nonrolling_tplus2') as syn_window_edge_selected_acc,

  max(v1) filter (where config_id = 'syn_cvd_price_divergence_velocity_15m5_tplus2') as syn_cvd_price_divergence_velocity,
  max(v2) filter (where config_id = 'syn_cvd_price_divergence_velocity_15m5_tplus2') as syn_cvd_div_cvd_norm,
  max(v3) filter (where config_id = 'syn_cvd_price_divergence_velocity_15m5_tplus2') as syn_cvd_div_price_norm,

  max(v1) filter (where config_id = 'syn_multitimeframe_trend_confluence_tplus2') as syn_multitimeframe_trend_confluence,
  max(v2) filter (where config_id = 'syn_multitimeframe_trend_confluence_tplus2') as syn_mtc_raw_score,
  max(v3) filter (where config_id = 'syn_multitimeframe_trend_confluence_tplus2') as syn_mtc_vol_filter
from x
group by pair, bucket_time;

comment on view indicators.v_synthetic_signal_inputs is
  'Serving view for signal scripts: pivots active synthetic indicators to one row per pair/bucket_time.';
