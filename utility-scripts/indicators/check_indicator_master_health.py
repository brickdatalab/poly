#!/usr/bin/env python3
"""Run unified indicator master health snapshot and emit AI-friendly JSON output."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ops.fn_indicator_master_health_snapshot")
    ...
    from (
      select ops.fn_indicator_master_health_snapshot(
    ...
    if args.tldr:
        ...
        if not non_pass:
            print("[indicator_master] no failures/warnings")
        else:
            print("[indicator_master] non-pass rows")
            for row in non_pass:
                print(
                    f"- {row.get('indicator_key')} pair={row.get('pair')} "
                    f"status={row.get('status')} light={row.get('traffic_light')} "
                    f"reason={row.get('reason_code')} lag={row.get('lag_seconds')}"
                )
    else:
        for row in out_rows:
            print(
                f"{row.get('status'):4} {row.get('indicator_key')} pair={row.get('pair')} "
                f"light={row.get('traffic_light')} reason={row.get('reason_code')}"
            )

    stamp = dt.datetime.now(dt.UTC).strftime("%Y%m%dT%H%M%SZ")
    payload = {
        "generated_at_utc": dt.datetime.now(dt.UTC).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "source": "indicator_master_health",
        "pairs": list(pairs),
        "window": {"type": "hours", "value": args.lookback_hours},
        "summary": summary,
        "rows": out_rows,
    }
    out_path = Path(__file__).resolve().parent / "output" / f"indicator_master_health_{args.lookback_hours}h_{stamp}.json"
