# Market Context Utility Runbook (AI Agent)

## Canonical Script
`/Users/vitolo/Desktop/projects/poly/utility-scripts/market_context/check_market_context_health.py`

## Primary Purpose
Verify `public.market_context` is fresh per pair and has continuous minute-level coverage in a rolling UTC window.
...
2. Full detail audit:
```bash
python3 utility-scripts/market_context/check_market_context_health.py --lookback-minutes 180
```

3. Custom pair set:
```bash
python3 utility-scripts/market_context/check_market_context_health.py --pairs BTC-USD,ETH-USD --lookback-minutes 180 --tldr
```
...
```bash
python3 utility-scripts/market_context/check_market_context_health.py --lookback-minutes 60 --tldr
```

## Output Artifacts

All run outputs are written to:
`/Users/vitolo/Desktop/projects/poly/utility-scripts/market_context/output/`

File pattern:
`market_context_health_<N>m_<UTCSTAMP>.json`
...
3. Freshness threshold comes from `ops.pipeline_slo_config` (`source='market_context'`), with fallback default of 120 seconds.
