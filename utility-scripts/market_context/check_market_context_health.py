#!/usr/bin/env python3
"""
Audit market_context source freshness and minute continuity for a rolling UTC window.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit market_context freshness + continuity")
    ...
    payload = build_output_payload(
        ...
        rows=rows,
        meta={"table": "public.market_context", "timestamp_column": "timestamp"},
    )
    out_dir = Path(__file__).resolve().parent / "output"
    out_dir.mkdir(parents=True, exist_ok=True)
    stamp = dt.datetime.now(dt.UTC).strftime("%Y%m%dT%H%M%SZ")
    out_path = out_dir / f"market_context_health_{args.lookback_minutes}m_{stamp}.json"
